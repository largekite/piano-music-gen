...
export default function ProgressDisplay({
  ...
  const getBgStyle = () => {
    ...
    return 'bg-white/80 border-warm-200';
  };

  return (
    <div className={`rounded-2xl border p-6 ${getBgStyle()} backdrop-blur shadow-sm transition-all`}>
      {/* Stage header */}
      <div className="flex items-center gap-3 mb-4">
        {/* Animated icon */}
        {isGenerating && !error && stage !== 'complete' && (
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-coral-400 to-warm-400 flex items-center justify-center shadow-sm">
        ...
        {error && (
          <div className="w-10 h-10 rounded-xl bg-coral-400 flex items-center justify-center shadow-sm">
            <svg className="w-5 h-5 text-white" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="flex-1">
          <h3 className="font-bold text-stone-700">{getStageLabel()}</h3>
      ...
      {isGenerating && !error && (
        <div className="space-y-2">
          <div className="w-full bg-warm-200 rounded-full h-2.5 overflow-hidden">
            <div
              className="bg-gradient-to-r from-coral-400 to-warm-400 h-2.5 rounded-full transition-all duration-500 ease-out"
              style={{ width: `${progress}%` }}
            />
          </div>
          <div className="text-right text-xs font-medium text-stone-400">
            {progress}%
          </div>
        </div>
      )}
